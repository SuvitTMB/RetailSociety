...
function ListRewards() {
  ...
  .get().then((snapshot)=> {
    ...
    if(doc.data().WheelRandom==1) {
      ...
      str += '<div class="font13black" style="color:#0056ff;">'+ doc.data().RewardsName +'</div>';
...
function CheckGifts() {
  ...
  var i = 1;
  var str = "";
  //str += '<div class="btn-t3" style="cursor: default;margin-top:10px;background:#fff;">รางวัลสำหรับหมุนวงล้อ</div>';
  //str += '<div id="DisplayAllStock"></div>';
  str += '<table class="table table-bordered" class="font13black" style="background-color: #fff;">';
  ...
  .get().then((snapshot)=> {
    snapshot.forEach(doc=> {
      ...
      str += '<td style="text-align:left;">'+ doc.data().giftname +' ('+ doc.data().gifttotal +')</td>';
...
function OpenLink(x,price,i) {
  ...
  .get().then((snapshot)=> {
    snapshot.forEach(doc=> {
      ...
      str += '<div class="font13black" style="color:#0056ff;">'+ results[0].RewardsName +'</div>';
  ...
  .get().then((snapshot)=> {
    snapshot.forEach(doc=> {
      if(doc.data().RewardsStock==0) {
        ...
        str += '<div class="font13black" style="color:#0056ff;">'+ xRewardsName +'</div>';
